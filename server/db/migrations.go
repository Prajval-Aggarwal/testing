...
func AutoMigrateDatabase(db *gorm.DB) {
	...
	if dbVersion.Version < 1 {
		err := db.AutoMigrate(&model.Player{}, &model.OwnedCars{}, &model.Car{}, &model.CarStats{}, &model.Garage{}, &model.GarageCarList{}, &model.GarageUpgrades{}, &model.OwnedGarage{}, &model.PlayerCarUpgrades{}, &model.PlayerCarsStats{})
		if err != nil {
			panic(err)
		}
	...
	if dbVersion.Version < 6 {
		err := db.AutoMigrate(&model.Arena{}, &model.ArenaReq{})
		if err != nil {
			panic(err)
		}
		db.Where("version=?", dbVersion.Version).Updates(&model.DbVersion{
